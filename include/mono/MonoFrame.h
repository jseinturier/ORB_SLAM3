#ifndef MOVS_MONO_FRAME_H
#define MOVS_MONO_FRAME_H
...
namespace ORB_SLAM3
{
    ...
    class MonoFrame : public Frame
    {
        ...
        // Copy constructor.
        MonoFrame(const MonoFrame& frame) : Frame(frame) {};

        // Constructor for Monocular cameras.
        MonoFrame(const cv::Mat& imGray, const double& timeStamp, 
                  ORBextractor* extractor, ORBVocabulary* voc, 
                  GeometricCamera* pCamera, cv::Mat& distCoef, 
                  const float& bf, const float& thDepth, Frame* pPrevF = static_cast<Frame*>(NULL)) : Frame(timeStamp, extractor, voc, pCamera, distCoef, bf, thDepth);
